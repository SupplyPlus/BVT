        ...
        if invoice_search:
            ...
            if self.type == "in" or self.type == "cus_credit_note":
                ...
                if partner_id.property_account_payable_id:
                    ...
                    account_search = self.env['ir.property'].sudo().search(
                        [('name', '=', 'property_account_payable_id')], limit=1)
                    account_id = account_search.value_reference
                    if not account_id:
            ...
            ref_ids = []
            if values.get('ref'):
                if ',' in values.get('ref'):
                    ref_names = values.get('ref').split(',')
                    for ref_name in ref_names:
                        ref = self.env['account.bill.reference'].search([('name', '=', ref_name)], limit=1)
                        if not ref:
                            bill_ref = self.env['account.bill.reference'].create({'name': ref_name})
                            if bill_ref:
                                ref_ids.append(bill_ref.id)
                        if ref:
                            ref_ids.append(ref.id)
                else:
                    ref = self.env['account.bill.reference'].search([('name', '=', values.get('ref'))], limit=1)
                    if not ref:
                        bill_ref = self.env['account.bill.reference'].create({'name': values.get('ref')})
                        ref_ids.append(bill_ref.id)
                    if ref:
                        ref_ids.append(ref.id)

            # purchase_tag_ids = []
            # if values.get('purchase_tag'):
            #     if ',' in values.get('purchase_tag'):
            #         purchase_tag_names = values.get('purchase_tag').split(',')
            #         for purchase_tag_name in purchase_tag_names:
            #             po_tag = self.env['purchase.tag'].search([('name', '=', purchase_tag_name)], limit=1)
            #             if not po_tag:
            #                 po_tags = self.env['purchase.tag'].create({'name': purchase_tag_name})
            #                 if po_tags:
            #                     purchase_tag_ids.append(po_tags.id)
            #             if po_tag:
            #                 purchase_tag_ids.append(po_tag.id)
            #     else:
            #         purchase_tag_names = self.env['purchase.tag'].search([('name', '=', values.get('purchase_tag'))],
            #                                                              limit=1)
            #         if not purchase_tag_names:
            #             po_tag = self.env['purchase.tag'].create({'name': values.get('purchase_tag')})
            #             purchase_tag_ids.append(po_tag.id)
            #         if purchase_tag_names:
            #             purchase_tag_ids.append(purchase_tag_names.id)
            ...
            inv_id = invoice_obj.create({
                ...
                'journal_id': self.journal_id.id if self.journal_id else journal.id,
                'invoice_name': values.get('invoice'),
                'ref_ids': ([(6, 0, ref_ids)]),
                # 'purchase_tags': ([(6, 0, purchase_tag_ids)]),
    ...
    def make_invoice_line(self, values, inv_id):
        ...
        if values.get('analytic_account'):
            analytic_acc_id = self.env['account.analytic.account'].search(
                [('name', '=', values.get('analytic_account'))])
            if analytic_acc_id:
        ...
        #     else:
        #         tag_names = values.get('analytic_tag').split(',')
        #         tag = self.env['account.analytic.tag'].search([('name', 'in', tag_names)])
        #         if not tag:
        #             raise ValidationError(_('"%s" Tag not in your system') % tag_names)
        #         tag_ids.append(tag.id)
        #
        #     if tag_ids:
        ...
        if product_id != 0:
            if  product_id.product_tmpl_id.uom_ids and product_id.product_tmpl_id.uom_ids.filtered(
                    lambda x: x.uom_product_type == 'uom_po_id'):
                purchase_uom_id = product_id.product_tmpl_id.uom_ids.filtered(
                    lambda x: x.uom_product_type == 'uom_po_id')
                product_uom = purchase_uom_id[0].id
                    ...
                    if self.account_opt == 'default':
                        ...
                        if len(line) >= 13:
                            ...
                            values.update({'invoice': line[0],
                                           ...
                                           'date': date_string,
                                           'account': line[12],
                                           'analytic_account': line[13],
                                           'analytic_tag': line[14],
                                           'seq_opt': self.sequence_opt,
                                           'ref': line[15],
                                           'purchase_tag': line[16],
                                           })
                        elif len(line) > 13:
                            raise ValidationError(_('Your File has extra column please refer sample file'))
                        else:
