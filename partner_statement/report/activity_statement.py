...
class ActivityStatement(models.AbstractModel):
    ...
    def _initial_balance_sql_q1(self, partners, date_start, account_type):
        ...
        return str(
            ...
            JOIN account_account aa ON (aa.id = l.account_id AND aa.account_type in('asset_receivable','liability_payable'))
    ...
    def _initial_balance_sql_q2(self, company_id):
        return str(
            self._cr.mogrify(
                """
            SELECT Q1.partner_id, sum(debit-credit) AS balance,
    ...
    def _display_lines_sql_q1(self, partners, date_start, date_end, account_type):
        ...
        return str(
            ...
            SELECT m.name AS move_id, l.partner_id, l.date,
                ...
                END as name,
                CASE
                    WHEN (aj.type IN ('purchase','sale','general')) AND l.name IS NOT NULL
                        THEN CONCAT(l.ref,' ',l.name)
                    WHEN aj.type IN ('purchase','sale','general') AND l.name IS NULL
                        THEN m.ref
                    WHEN (aj.type in ('bank', 'cash'))
                        THEN CONCAT('Payment ',l.name)
                    ELSE ''
                END as ref,
            ...
            GROUP BY l.partner_id, m.name, l.date, l.date_maturity,
                ...
                END,
                CASE
                    WHEN (aj.type IN ('sale','purchase','general')) AND l.name IS NOT NULL
                        THEN l.ref
                    WHEN aj.type IN ('sale','purchase','general') AND l.name IS NULL
                        THEN m.ref
                    WHEN (aj.type in ('bank','cash'))
                        THEN 'Payment'
                    ELSE ''
                END,
                l.blocked, l.currency_id, l.amount_currency, l.company_id,aj.type,l.name,l.ref,m.ref
        """,
                locals(),
            ),
            "utf-8",
